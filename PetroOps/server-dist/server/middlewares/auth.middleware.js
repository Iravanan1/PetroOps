...
const authMiddleware = (allowedRoles) => {
    return async (req, res, next) => {
        try {
            ...
            let decodedToken;
            try {
                decodedToken = await firebaseAdmin_js_1.adminAuth.verifyIdToken(idToken);
            }
            catch (err) {
                if (isProduction) {
            ...
            let userData = {};
            try {
                const userDoc = await firebaseAdmin_js_1.adminDb.collection('users').doc(decodedToken.uid).get();
