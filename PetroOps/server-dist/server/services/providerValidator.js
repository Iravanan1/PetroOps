...
const EnvironmentSchema = zod_1.z.object({
    GEMINI_API_KEY: zod_1.z.string().min(10, "Gemini API key must be a valid key"),
    CLAUDE_API_KEY: zod_1.z.string().min(5, "Claude API key is required"),
    OPENAI_API_KEY: zod_1.z.string().min(5, "OpenAI API key is required"),
});
class ProviderValidator {
    ...
    static async runConnectivityTests() {
        ...
        try {
            ...
            const hasTorch = fs_1.default.existsSync(path_1.default.join(process.cwd(), "venv/bin/python"));
            results.push({
                provider: "Local OCR",
    ...
    static writeConnectivityReport(results) {
        const reportPath = path_1.default.join(this.artifactsDir, "provider_connectivity_report.md");
