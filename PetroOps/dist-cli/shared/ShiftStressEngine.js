...
exports.ShiftStressEngine = exports.FUEL_RATES = void 0;
const TransactionNormalizer_1 = require("../accounting/TransactionNormalizer");
const CoreReplayEngine_1 = require("../replay/CoreReplayEngine");
...
let seqCounter = 0;
function makeEvent(type, shiftId, operatorId, message, payload = {}, severity = 'info', failureMode = 'none') {
...
// ─── SIMULATION ENGINE ─────────────────────────────────────────────────────────────
class ShiftStressEngine {
    // ── Shift Generator ────────────────────────────────────────────────────────────
    static generateShift(date, label, injectFailures = [], carryForwardCash = 0) {
        ...
        // ── Nozzle setup
        const nozzles = [
            { id: 'N1', label: 'Nozzle 1', fuel: 'MS', opening: parseFloat(rand(1200, 2000).toFixed(2)), closing: 0, testing: parseFloat(rand(1, 5).toFixed(2)), rate: exports.FUEL_RATES.MS },
            { id: 'N2', label: 'Nozzle 2', fuel: 'MS', opening: parseFloat(rand(1200, 2000).toFixed(2)), closing: 0, testing: parseFloat(rand(1, 5).toFixed(2)), rate: exports.FUEL_RATES.MS },
            { id: 'N3', label: 'Nozzle 3', fuel: 'HSD', opening: parseFloat(rand(800, 1500).toFixed(2)), closing: 0, testing: parseFloat(rand(1, 4).toFixed(2)), rate: exports.FUEL_RATES.HSD },
            { id: 'N4', label: 'Nozzle 4', fuel: 'HSD', opening: parseFloat(rand(800, 1500).toFixed(2)), closing: 0, testing: parseFloat(rand(1, 4).toFixed(2)), rate: exports.FUEL_RATES.HSD },
        ];
        ...
        events.push(makeEvent('SHIFT_OPEN', shiftId, operator.id, `Shift opened. Operator: ${operator.name}. Opening float: ₹${openingCash.toLocaleString('en-IN')}`, { openingCash, operator }, 'info'));
        ...
        for (let t = 0; t < txCount; t++) {
            ...
            if (txType === 'upi') {
                ...
                upiTotal += amt;
                events.push(makeEvent('UPI_COLLECT', shiftId, operator.id, `UPI collected ₹${amt.toLocaleString('en-IN')} via QR terminal`, { amount: amt }));
            }
            else if (txType === 'card') {
                const amt = randInt(500, 8000);
                cardTotal += amt;
                events.push(makeEvent('CARD_COLLECT', shiftId, operator.id, `Card payment ₹${amt.toLocaleString('en-IN')} via POS`, { amount: amt }));
            }
            else if (txType === 'credit') {
                const amt = randInt(300, 3000);
                creditSales += amt;
                events.push(makeEvent('CREDIT_SALE', shiftId, operator.id, `Credit sale ₹${amt.toLocaleString('en-IN')} (udhari customer)`, { amount: amt }));
            }
            else if (txType === 'expense') {
                const desc = pick(EXPENSE_TEMPLATES);
                const amt = randInt(100, 2000);
                expenses += amt;
                events.push(makeEvent('EXPENSE', shiftId, operator.id, `Expense: ${desc} ₹${amt.toLocaleString('en-IN')}`, { desc, amount: amt }));
            }
            else if (txType === 'recovery') {
                const amt = randInt(200, 2500);
                creditRecovery += amt;
                events.push(makeEvent('CREDIT_RECOVERY', shiftId, operator.id, `Credit recovery ₹${amt.toLocaleString('en-IN')} collected`, { amount: amt }));
            }
        }
        ...
        events.push(makeEvent('DIP_READING', shiftId, operator.id, `Dip readings recorded: MS1=${dipReadings.MS_tank_1}L MS2=${dipReadings.MS_tank_2}L HSD=${dipReadings.HSD_tank_1}L`, { dipReadings }));
        ...
        for (const failure of injectFailures) {
            switch (failure) {
                case 'till_shortage': {
                    ...
                    shortage = short;
                    events.push(makeEvent('STRESS_SCENARIO', shiftId, operator.id, `INJECTED till shortage of ₹${short.toLocaleString('en-IN')}`, { failure, shortage: short }, 'error', 'till_shortage'));
                    break;
                }
                case 'duplicate_entry': {
                    const dupAmt = randInt(500, 3000);
                    upiTotal += dupAmt; // same entry added twice
                    events.push(makeEvent('DUPLICATE_ENTRY', shiftId, operator.id, `INJECTED duplicate UPI entry ₹${dupAmt.toLocaleString('en-IN')} — double-posted`, { failure, dupAmt }, 'warn', 'duplicate_entry'));
                    break;
                }
                case 'ocr_failure': {
                    events.push(makeEvent('OCR_FAILURE', shiftId, operator.id, `INJECTED OCR extraction failure — nozzle readings unreadable, falling back to manual`, { failure, affectedNozzles: ['N1', 'N3'] }, 'error', 'ocr_failure'));
                    break;
                }
                case 'nozzle_rollback': {
                    ...
                    events.push(makeEvent('NOZZLE_ROLLBACK', shiftId, operator.id, `INJECTED nozzle rollback on ${rollbackNozzle.label}: closing reset from ${oldClosing.toFixed(2)} to ${rollbackNozzle.closing.toFixed(2)}`, { failure, nozzleId: rollbackNozzle.id, oldClosing, newClosing: rollbackNozzle.closing }, 'error', 'nozzle_rollback'));
                    break;
                }
                case 'carry_forward_mismatch': {
                    const delta = randInt(100, 800);
                    events.push(makeEvent('CARRY_FORWARD', shiftId, operator.id, `INJECTED carry-forward mismatch: expected ₹${carryForwardCash.toLocaleString('en-IN')}, actual ₹${(carryForwardCash - delta).toLocaleString('en-IN')}`, { failure, expected: carryForwardCash, actual: carryForwardCash - delta, delta }, 'error', 'carry_forward_mismatch'));
                    break;
                }
                case 'offline_conflict': {
                    events.push(makeEvent('SYNC_CONFLICT', shiftId, operator.id, `INJECTED sync conflict: same shiftId submitted twice from offline queue during network restore`, { failure, conflictType: 'duplicate_shiftId' }, 'warn', 'offline_conflict'));
                    break;
                }
                case 'partial_write': {
                    events.push(makeEvent('OFFLINE_WRITE', shiftId, operator.id, `INJECTED partial write: IndexedDB transaction interrupted mid-batch (simulated power loss)`, { failure, batchProgress: '43/100' }, 'critical', 'partial_write'));
                    break;
                }
                case 'delayed_settlement': {
                    events.push(makeEvent('STRESS_SCENARIO', shiftId, operator.id, `INJECTED delayed settlement: UPI gateway response pending for 47s (timeout threshold 30s)`, { failure, gatewayDelayMs: 47000 }, 'warn', 'delayed_settlement'));
                    break;
                }
                case 'duplicate_sync': {
                    events.push(makeEvent('OFFLINE_REPLAY', shiftId, operator.id, `INJECTED duplicate sync: offline queue replayed same transaction x3 before dedup detection`, { failure, duplicates: 3, txId: `tx_${Date.now()}` }, 'warn', 'duplicate_sync'));
                    break;
                }
            }
        }
        ...
        events.push(makeEvent('SNAPSHOT_CHECKPOINT', shiftId, operator.id, `Snapshot checkpoint generated: hash=${snapshotHash}`, { snapshotHash, ...snapshotPayload }, 'info'));
        // ── FSM lifecycle
    ...
    // ── Multi-Day Replay ────────────────────────────────────────────────────────────
    static generateMultiDayReplay(days, failureRate = 0.2) {
        const shifts = [];
    ...
    // ── Load Test ──────────────────────────────────────────────────────────────────
    static generateLoadTest(txTarget = 1000) {
        const shifts = [];
        let totalTx = 0;
        let carryForward = 15000;
        const today = new Date();
    ...
    static validateReplay(shift) {
        const issues = [];
        let upi = 0, card = 0, credit = 0, recovery = 0, expense = 0;
        for (const ev of shift.events) {
            if (ev.type === 'UPI_COLLECT')
                upi += ev.payload.amount || 0;
            if (ev.type === 'CARD_COLLECT')
                card += ev.payload.amount || 0;
            if (ev.type === 'CREDIT_SALE')
                credit += ev.payload.amount || 0;
            if (ev.type === 'CREDIT_RECOVERY')
                recovery += ev.payload.amount || 0;
            if (ev.type === 'EXPENSE')
                expense += ev.payload.amount || 0;
        }
        ...
        if (upi !== shift.upiTotal) {
            ...
            if (dupDelta > 1)
                issues.push(`UPI replay mismatch: replayed ₹${upi.toFixed(2)}, recorded ₹${shift.upiTotal.toFixed(2)}`);
        }
        // Verify checksums
        ...
        if (!matches)
            issues.push(`Snapshot hash mismatch — data mutated after locking`);
        return { valid: issues.length === 0, recomputedHash, matches, issues };
    }
    // ── Conversion Helper ──────────────────────────────────────────────────────────
    static mapSimShiftToAIExtraction(shift) {
        return {
    ...
    static runAccountingReplay(branchId, shifts, simMode = 'operator') {
        const carryForwardChecks = [];
        const nozzleContinuityChecks = [];
        const settlementChecks = [];
        const lockedPeriodViolations = [];
        const mismatchReport = [];
        const correctionRecommendations = [];
        ...
        sortedShifts.forEach((s, idx) => {
            ...
            if (hasShortage && simMode === 'manager') {
                s.events.push(makeEvent('STRESS_SCENARIO', s.id, 'mgr_001', `[MANAGER OVERRIDE APPROVED] Shortage of ₹${s.shortage.toLocaleString('en-IN')} approved by supervisor Mohan Gupta. Reconciliation adjustment posted.`, { override: true }, 'info'));
            }
            const normalized = TransactionNormalizer_1.TransactionNormalizer.normalizeShiftToTransactions(branchId, aiExtraction, currentSeq);
        ...
        const replayState = CoreReplayEngine_1.CoreReplayEngine.replayLedger(branchId, transactions);
    ...
    // ── Compute Metrics ────────────────────────────────────────────────────────────
    static computeMetrics(run, replayResults, accountingReplay) {
    ...
    // ── Full Stress Run ─────────────────────────────────────────────────────────────
    static runStressTest(mode, config = {}) {
...
exports.ShiftStressEngine = ShiftStressEngine;
exports.default = ShiftStressEngine;
