...
export class OcrService {
  public static async extractText(imageBuffer: Buffer): Promise<{ text: string, confidence: number }> {
    try {
      // 1. Preprocess Image
      console.log("Preprocessing image...");
      const enhancedBuffer = await ImageService.preprocessForOCR(imageBuffer);
      ...
      try {
        ...
        const resultWrapper: any = await retryWithBackoff(() => 
          visionClient.documentTextDetection({ image: { content: enhancedBuffer } })
        );
  ...
  private static cleanOcrNoise(raw: string): string {
    return raw
      .replace(/[O]/g, "0") 
      .replace(/[I|l]/g, "1")
      .replace(/₹\s*/g, "INR ")
