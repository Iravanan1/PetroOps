...
import { TransactionPayload } from "./FirestoreSecurityValidationEngine";

export interface ChainedTransaction extends TransactionPayload {
  transactionId: string;
  previousHash: string; // Hash of preceding transaction in sequence
  currentHash: string;  // Rolling hash of this + previousHash
  sequenceNumber: number;
}

export class ImmutableLedgerGuard {
  /**
   * Generates a deterministic hash for a transaction node
   */
  public static calculateNodeHash(
    previousHash: string,
   ...
   */
  public static verifyChainIntegrity(chain: ChainedTransaction[]): {
    isCorrupted: boolean;
    corruptedSequenceIndices: number[];
  } {
    if (chain.length === 0) {
      return { isCorrupted: false, corruptedSequenceIndices: [] };
    }

    const corruptedSequenceIndices: number[] = [];

    // First node checks
    const firstNode = chain[0];
    const calculatedFirstHash = this.calculateNodeHash(
      firstNode.previousHash,
      firstNode,
      firstNode.sequenceNumber
    );
    ...
    for (let i = 1; i < chain.length; i++) {
      ...
      const reCalculatedHash = this.calculateNodeHash(
        currentNode.previousHash,
        currentNode,
        currentNode.sequenceNumber
      );
