/**
 * ReplayChecksumService
 * ─────────────────────
 * Provides cryptographic SHA-256 rolling checksum computation for the
 * append-only ledger replay chain.  A single character change in any past
 * transaction immediately breaks every downstream hash, making tampering
 * trivially detectable.
 *
 * Environments:
 *  • Browser  – uses the native `SubtleCrypto` API (no dependencies).
 *  • Node/SSR – falls back to a deterministic djb2-32 hash (same algorithm
 *               used elsewhere in the replay engine for offline operation).
 */
export interface ChecksumBlock {
  blockIndex: number;
  transactionId: string;
  previousChecksum: string;
  payload: string;               // stringified deterministic content
  checksum: string;              // SHA-256 hex (browser) or djb2 hex (node)
  computedAt: string;
}

export interface ChecksumChainVerification {
  isIntact: boolean;
  totalBlocks: number;
  corruptedAt: number | null;    // block index where chain first broke
  corruptedTransactionId: string | null;
  errors: string[];
}

export class ReplayChecksumService {
  /**
   ...
   */
  public static djb2Hash(content: string): string {
    let hash = 5381;
   ...
   */
  public static async buildBlock(
    blockIndex: number,
    transactionId: string,
    previousChecksum: string,
    transactionPayload: string
  ): Promise<ChecksumBlock> {
   ...
   */
  public static async buildGenesisBlock(branchId: string): Promise<ChecksumBlock> {
   ...
   */
  public static async verifyChain(blocks: ChecksumBlock[]): Promise<ChecksumChainVerification> {
    const errors: string[] = [];
    let corruptedAt: number | null = null;
    let corruptedTransactionId: string | null = null;
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (block.checksum !== expectedChecksum) {
        ...
        errors.push(
          `[Chain Corruption] Block ${i} (txId: ${block.transactionId}) ` +
      ...
      if (i > 0 && block.previousChecksum !== blocks[i - 1].checksum) {
        ...
        errors.push(
          `[Chain Break] Block ${i} previousChecksum (${block.previousChecksum}) ` +
      ...
      if (block.blockIndex !== i) {
        errors.push(
          `[Index Mismatch] Block at position ${i} has blockIndex=${block.blockIndex}. ` +
   ...
   */
  public static generateIdempotencyKey(fields: {
    branchId: string;
    date: string;
    debitAccount: string;
