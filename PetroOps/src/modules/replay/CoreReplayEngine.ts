import { LedgerTransaction, TransactionNormalizer } from '../accounting/TransactionNormalizer';
...
export class CoreReplayEngine {
  /**
   * List of asset/expense account types that increase with Debit entries.
   */
  private static DEBIT_INCREASE_ACCOUNTS = new Set([
    'Cash Till',
    'UPI Clearing',
    'Card Clearing',
    'Accounts Receivable',
   ...
   */
  private static CREDIT_INCREASE_ACCOUNTS = new Set([
    'Fuel Revenue'
  ]);
   ...
   */
  public static replayLedger(
    branchId: string,
    transactions: LedgerTransaction[],
    initialBalances: Record<string, number> = {},
    initialChecksum: string = `seed_${branchId}`
  ): ReplayState {
    ...
    sortedTxs.forEach((tx, idx) => {
      ...
      const computedHash = TransactionNormalizer.computeTransactionHash(tx);
