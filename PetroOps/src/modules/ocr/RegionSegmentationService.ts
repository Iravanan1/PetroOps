...
import { RegisterLayoutSegmentationEngine } from './segmentation/RegisterLayoutSegmentationEngine';
import { TokenReductionEngine } from '../ai/inference/TokenReductionEngine';

export interface BoundedRegion {
  regionId: string;
  name: 'nozzle_block' | 'payment_block' | 'totals_block' | 'expenses_block' | 'handwritten_block' | 'wetstock_block';
  x: number;
  y: number;
  width: number;
  height: number;
...
export interface ImageQualityGrade {
  blurScore: number;         // 0 (extremely blurry) to 100 (perfectly sharp)
  skewDegrees: number;       // degrees of camera tilt, ideally 0
  lightingUniformity: number; // 0 (extreme shadows) to 100 (perfectly lit)
  contrastRatio: number;      // 0 (washed out) to 100 (perfect high-contrast)
  overallGrade: number;       // weighted index 0 to 100
}

export class RegionSegmentationService {
  /**
   ...
   */
  public static gradeImageQuality(
    imageWidth: number,
    imageHeight: number,
   ...
   */
  public static analyzeHandwritingDensity(ocrContent: string): { isHandwritten: boolean; handwritingDensity: number } {
   ...
   */
  public static segmentImage(imageWidth: number, imageHeight: number, ocrContent: string): BoundedRegion[] {
    const w = imageWidth || 1200;
    const h = imageHeight || 1800;
    ...
    const nozzleHash = ReplaySafeImportService.computeOcrHash(nozzleLines.join(" "));
    const paymentHash = ReplaySafeImportService.computeOcrHash(paymentLines.join(" "));
    const expenseHash = ReplaySafeImportService.computeOcrHash(expenseLines.join(" "));
    const wetstockHash = ReplaySafeImportService.computeOcrHash(wetstockLines.join(" "));

    // Offload complex spatial reasoning to the new Layout Segmentation Engine
    const { skewDegrees } = this.gradeImageQuality(w, h);
    const spatialData = RegisterLayoutSegmentationEngine.segmentRegisterImage(ocrContent, w, h, skewDegrees);

    const regions: BoundedRegion[] = [
      {
      ...
      },
      {
        ...
        contentHash: ReplaySafeImportService.computeOcrHash(ocrContent.slice(-100)),
    ...
    if (isHandwritten || spatialData.rawHandwritingBlobs.length > 0) {
      regions.push({
        ...
        contentHash: ReplaySafeImportService.computeOcrHash("handwritten_annotation_segment"),
   ...
   */
  public static extractFocusedRegionText(ocrContent: string, regionName: BoundedRegion['name']): string {
    ...
    return TokenReductionEngine.minifyRawText(filteredText);
  }
}
