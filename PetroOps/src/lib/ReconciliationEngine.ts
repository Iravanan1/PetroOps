...
export interface NozzleVariance {
  id: number;
  fuel: FuelType;
  saleLitres: number;
  expectedSale: number;
...
export interface ReconciliationReport {
  ...
  shiftId: string;
  metrics: {
    totalLitres: { hsd: number; ms: number };
    revenue: number;
    expectedCash: number;
    actualCash: number;
    cashMismatch: number;
  ...
  variances: {
    nozzles: NozzleVariance[];
    tanks: { hsd: number; ms: number };
  };
  flags: {
    level: 'info' | 'warn' | 'error' | 'theft';
    message: string;
  }[];
}

export class ReconciliationEngine {
  public static reconcile(shift: ShiftRecord): ReconciliationReport {
    const hS = DEFAULT_RATES.hsdSell;
    const mS = DEFAULT_RATES.msSell;
    const hB = DEFAULT_RATES.hsdBuy;
    const mB = DEFAULT_RATES.msBuy;
  ...
  private static auditNozzles(nozzles: NozzleReading[]) {
    let totalSales = 0;
    const variances: NozzleVariance[] = nozzles.map(n => {
  ...
  private static sumEntries(entries: string[]): number {
    return entries.reduce((sum, val) => sum + (+val || 0), 0);
  }

  private static sumMoneyEntries(entries: MoneyEntry[]): number {
    return entries.reduce((sum, e) => sum + (+e.amount || 0), 0);
  }

  private static generateFlags(cashDiff: number, hDiff: number, mDiff: number) {
    const flags: { level: 'info' | 'warn' | 'error' | 'theft'; message: string }[] = [];
    const absCash = Math.abs(cashDiff);

    if (absCash >= THRESHOLDS.CASH_THEFT) {
      flags.push({ level: 'theft', message: `Critical Cash Mismatch: INR ${absCash.toLocaleString()}` });
    } else if (absCash >= THRESHOLDS.CASH_WARN) {
      flags.push({ level: 'warn', message: `Cash Variance detected: INR ${absCash.toLocaleString()}` });
    }

    if (Math.abs(hDiff) >= THRESHOLDS.TANK_WARN) {
      flags.push({ level: 'warn', message: `HSD Tank Mismatch: ${hDiff.toFixed(2)}L` });
    }

    if (Math.abs(mDiff) >= THRESHOLDS.TANK_WARN) {
