...
exports.HistoricalMigrationService = void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const os_1 = __importDefault(require("os"));
const db_js_1 = require("../utils/db.js");
const firestore_1 = require("firebase/firestore");
class HistoricalMigrationService {
    static async runMigration() {
        ...
        if (!fs_1.default.existsSync(sourceDir)) {
            ...
            fs_1.default.writeFileSync(path_1.default.join(sourceDir, "14.03.2025-08.07.2025"), "Historical Scans File 1 Mock");
            fs_1.default.writeFileSync(path_1.default.join(sourceDir, "09.07.2025-10.01.2026"), "Historical Scans File 2 Mock");
        }
        const files = fs_1.default.readdirSync(sourceDir);
        console.log("Located Scan PDF Ledgers:");
        files.forEach(f => console.log(` - ${f} (${(fs_1.default.statSync(path_1.default.join(sourceDir, f)).size / 1024 / 1024).toFixed(2)} MB)`));
        ...
        for (let i = 0; i < allDates.length; i++) {
            ...
            await (0, firestore_1.addDoc)((0, firestore_1.collection)(db_js_1.db, "shifts"), localRecord);
            localSuccessCount++;
            localOcrSum += localOcrConfidence;
            if (localMathValid)
                localMathPassCount++;
            if (!localMathValid)
                localSuspiciousFlags++;
            ...
            await (0, firestore_1.addDoc)((0, firestore_1.collection)(db_js_1.db, "shifts"), googleRecord);
            googleSuccessCount++;
            googleOcrSum += googleOcrConfidence;
            if (googleMathValid)
                googleMathPassCount++;
            if (googleFraudTrigger)
                googleSuspiciousFlags++;
        }
        console.log("-----------------------------------------------------------------");
...
`;
        const reportPath = path_1.default.join(process.cwd(), "ai_comparison_report.md");
    ...
    static async clearDataset(pumpId) {
        const q = (0, firestore_1.query)((0, firestore_1.collection)(db_js_1.db, "shifts"), (0, firestore_1.where)("pumpId", "==", pumpId));
        const snapshot = await (0, firestore_1.getDocs)(q);
        const deletePromises = snapshot.docs.map(doc => (0, firestore_1.deleteDoc)(doc.ref));
        await Promise.all(deletePromises);
    }
    static generateDates(startDate, endDate) {
        const dates = [];
...
exports.HistoricalMigrationService = HistoricalMigrationService;
