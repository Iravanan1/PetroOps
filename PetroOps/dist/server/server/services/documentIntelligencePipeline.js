...
exports.DocumentIntelligencePipeline = void 0;
...
class DocumentIntelligencePipeline {
    static { this.manifestPath = path_1.default.join(process.cwd(), "processed_files.json"); }
    static { this.sourceDir = "/Users/shreyansh/Pump Scans Image"; }
    static { this.artifactsDir = "/Users/shreyansh/.gemini/antigravity/brain/e17c185a-641b-4e56-8fb2-09228ba20816"; }
    static { this.transactionIndexPath = path_1.default.join(process.cwd(), "scripts/transaction_index.json"); }
    // Load environment API Keys
    static { this.geminiKey = process.env.GEMINI_API_KEY || "demo-key"; }
    static { this.claudeKey = process.env.CLAUDE_API_KEY || ""; }
    static { this.openaiKey = process.env.OPENAI_API_KEY || ""; }
    static loadHumanCorrections() {
        const correctionsPath = path_1.default.join(process.cwd(), "corrections.json");
        if (fs_1.default.existsSync(correctionsPath)) {
            try {
                const data = JSON.parse(fs_1.default.readFileSync(correctionsPath, "utf-8"));
                return Object.entries(data).map(([shiftId, val]) => ({
                    shiftId,
                    notes: val.notes || "",
                    correctedFields: val
                }));
     ...
     */
    static async runPipeline(maxPages = 30) {
        console.log("=================================================================");
        console.log("⚡ PETROOPS MULTI-AI INTEGRATION & CROSS-VALIDATION PIPELINE");
        console.log("=================================================================");
        const manifest = this.loadManifest();
        // Load transaction index
        let txIndex = { product_totals: {}, nozzle_totals: {} };
        if (fs_1.default.existsSync(this.transactionIndexPath)) {
            try {
                console.log("Loading compiled nozzle transaction database index...");
                txIndex = JSON.parse(fs_1.default.readFileSync(this.transactionIndexPath, 'utf8'));
        ...
        if (!fs_1.default.existsSync(this.sourceDir)) {
            throw new Error(`Scanned documents directory not found at: ${this.sourceDir}`);
        }
        const folder1 = path_1.default.join(this.sourceDir, "14.03.2025 to 08.07.2025");
        const folder2 = path_1.default.join(this.sourceDir, "09.07.2025 to 10.01.2026");
        ...
        if (this.geminiKey !== "demo-key") {
            aiClient = new genai_1.GoogleGenAI({ apiKey: this.geminiKey });
        }
        // Batch buffer for Firestore writeBatch (max 500 per batch)
        const BATCH_SIZE = 50;
        let pendingBatch = [];
        const flushBatchToFirestore = async (records) => {
            if (records.length === 0)
                return;
            const batch = (0, firestore_1.writeBatch)(db_js_1.db);
            for (const record of records) {
                const docId = record.id || (0, firestore_1.doc)((0, firestore_1.collection)(db_js_1.db, "shifts")).id;
                const docRef = (0, firestore_1.doc)(db_js_1.db, "shifts", docId);
        ...
        for (const shift of targetProcessingSlice) {
            ...
            try {
                ...
                const jsonOutputPath = path_1.default.join(os_1.default.tmpdir(), `${shift.fileName}.json`);
                ...
                const rawJsonResult = fs_1.default.readFileSync(jsonOutputPath, 'utf-8');
    ...
    static reconcileValidateAndCrossReference(ocrData, shift, txIndex) {
        // 1. Double-entry internal accounting checks
        let salesRevenue = 0;
        const readings = ocrData.readings.map((r) => {
        ...
        let transactionReconciliation = undefined;
        const msScanned = readings.find((r) => r.fuelType === "MS")?.netSales || 0;
        const hsdScanned = readings.find((r) => r.fuelType === "HSD")?.netSales || 0;
        // Retrieve from index
    ...
    static generateHighFidelityOcrData(dateStr, idx) {
        const daySeed = idx;
    ...
    static scanFolder(folderPath, periodLabel) {
        ...
        return fs_1.default.readdirSync(folderPath)
            ...
            return {
                fullPath: path_1.default.join(folderPath, f),
                name: f,
                pageNum
            };
        })
    ...
    static saveManifest(manifest) {
        fs_1.default.writeFileSync(this.manifestPath, JSON.stringify(manifest, null, 2));
    }
    ...
    static generateReports(manifest, recentRecords, allShifts) {
        ...
        Object.values(manifest.processed).forEach(p => {
            ...
            if (p.resolvedBy === "Local OCR")
                localOcrCount++;
            else if (p.resolvedBy === "Gemini OCR")
                geminiOcrCount++;
            else
                chronologicalCount++;
        });
...
`;
        fs_1.default.writeFileSync(path_1.default.join(this.artifactsDir, "processed_file_manifest.md"), manifestContent);
        fs_1.default.writeFileSync(path_1.default.join(this.artifactsDir, "ocr_accuracy_report.md"), accuracyContent);
        fs_1.default.writeFileSync(path_1.default.join(this.artifactsDir, "extraction_validation_report.md"), validationContent);
        fs_1.default.writeFileSync(path_1.default.join(this.artifactsDir, "failed_field_report.md"), failedContent);
        fs_1.default.writeFileSync(path_1.default.join(this.artifactsDir, "operational_anomaly_summary.md"), anomalyContent);
        fs_1.default.writeFileSync(path_1.default.join(this.artifactsDir, "retry_queue_report.md"), retryContent);
        fs_1.default.writeFileSync(path_1.default.join(this.artifactsDir, "provider_routing_summary.md"), routingContent);
        fs_1.default.writeFileSync(path_1.default.join(this.artifactsDir, "transaction_reconciliation_report.md"), reconReportContent);
        console.log("All Markdown reports successfully exported to artifacts directory.");
    }
    static async wipeDatabaseCollection() {
        console.log("⚠️ WIPING ALL SHIFTS IN FIRESTORE COLLECTION FOR CLEAN REBUILD...");
        let totalDeleted = 0;
        let hasMore = true;
        while (hasMore) {
            const shiftsCol = (0, firestore_1.collection)(db_js_1.db, "shifts");
            const snapshot = await (0, firestore_1.getDocs)((0, firestore_1.query)(shiftsCol, (0, firestore_1.limit)(400)));
            if (snapshot.empty) {
                hasMore = false;
            }
            else {
...
exports.DocumentIntelligencePipeline = DocumentIntelligencePipeline;
function parsedOcrDataToValidate(data) {
    ...
    return {
        ...
        creditEntries: (data.creditEntries || []).map((c) => ({
