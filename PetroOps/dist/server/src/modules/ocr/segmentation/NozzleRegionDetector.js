...
exports.NozzleRegionDetector = void 0;
class NozzleRegionDetector {
    /**
     * Identifies the bounding area specific to the nozzle / wetstock section.
     */
    static locateNozzleBlock(tableGrids) {
        ...
        tableGrids.forEach(grid => {
            ...
            if (grid.y < minY)
                minY = grid.y;
            if (grid.x + grid.w > maxX)
                maxX = grid.x + grid.w;
            if (grid.y + grid.h > maxY)
                maxY = grid.y + grid.h;
        });
        return {
            x: minX,
...
exports.NozzleRegionDetector = NozzleRegionDetector;
